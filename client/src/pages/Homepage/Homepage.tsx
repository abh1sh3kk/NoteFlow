import React from "react";
import { AiOutlinePlus } from "react-icons/ai";
import { CiSearch } from "react-icons/ci";
import Note from "../../components/Note/Note";

function Homepage() {
    return (
        <div className="w-full flex justify-center ">
            <section className="w-full max-w-[965px] flex flex-col gap-4 px-4 lg:px-0">
                <div className="create-note-area flex justify-between">
                    <div className="create-area my-4">
                        <button className="flex justify-center items-center gap-2 bg-purple-500 text-white py-2 ps-3 pe-5 rounded-md">
                            <AiOutlinePlus />
                            <span>Create a note</span>
                        </button>
                    </div>
                    <div className="search-area relative flex justify-center items-center gap-2 ">
                        <CiSearch className="relative left-8" />
                        <input
                            placeholder="Search..."
                            className="ps-8 h-8 text-sm rounded-md outline-transparent border-none focus:outline-purple-400 focus:outline-[1px]"
                        />
                    </div>
                </div>
                <div className="note-container content-center grid gap-4 grid-cols-[repeat(auto-fill,minmax(296px,1fr))]">
                {/* <div className="note-container grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:grid-cols-[]"> */}
                    <Note />
                    <Note />
                    <Note />
                    <Note />
                    <Note />
                    <Note />
                </div>
            </section>
        </div>
    );
}

export default Homepage;
